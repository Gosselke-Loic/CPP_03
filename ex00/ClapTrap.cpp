...
/*   Updated: 2024/04/09 10:12:16 by lgosselk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::~ClapTrap( void ) {
	std::cout << "[PUB] All our Claptraps are made from 100% recycled materials [PUB]" \
		<< std::endl << RED << "[INFO] Claptrap: " << this->getName() << " has been recycled" \
		<< DEFAULT << std::endl; 
}
...
void	ClapTrap::takeDamage( unsigned int amount ) {
	if (this->getHitPoints() == 0) {
		std::cout << "Stop! Claptrap: " << this->getName() << " is already dead!" \
			<< std::endl;
	} else {
		std::cout << "Claptrap: " << this->getName() << " takes " << RED \
...
void	ClapTrap::beRepaired( unsigned int amount ) {
	if (this->getEnergyPoints() > 0) {
		std::cout << GREEN << "Claptrap: " << this->getName() << " has been repaired" \
			<< DEFAULT << std::endl;
		this->setHitPoints(this->getHitPoints() + (int)amount);
		this->setEnergyPoints(this->getEnergyPoints() - 1);
	}
	else {
		std::cout << "Claptrap: " << this->getName() << " have not enough energy points" \
			<< std::endl;
	}
}
...
void	ClapTrap::attack( const std::string& target ) {
	if (this->getEnergyPoints() > 0) {
		std::cout << "Claptrap: " << this->getName() << " attack and made " \
